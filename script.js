...
if(cvLink){
  cvLink.addEventListener('click', async function(e){
    ...
    try{
      const resp = await fetch(hrefVal, { method: 'HEAD' });
      if(!resp.ok){
        e.preventDefault();
        alert('Resume file not found at "' + hrefVal + '".\n\nPlease copy your PDF into the site folder at `assets/resume.pdf` (filename: resume.pdf) or update the download link to point to your resume.');
      }
    }catch(err){
